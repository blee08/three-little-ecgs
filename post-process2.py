...
def find_gaps(rpeaks, sampling_rate):
    # Heart rate < 30 BPM (RR > 2.0s) or > 600 BPM (RR < 0.1s)
    MAX_RR_LIMIT = 2.0 * sampling_rate
    MIN_RR_LIMIT = 0.1 * sampling_rate
    ...
    threshold = median + mad/0.6745 * 4.5
...
def calculate_time_series_metrics(
    gaps: List[P], event_timestamps: List[int], sampling_rate, num_series: int = 1
) -> List[Dict[str, Any]]:
    """
      ...
      while current_gap_index < len(gaps):
        ...
        if gap_end > window_start:
          ...
          if clip_end > clip_start:
            window_gap_overlaps.append(P.openclosed(clip_start, clip_end))
...
def plot_bpm(result, fname, sampling_rate):
...
def get_representative_item(window_start, items, sampling_rate):
  ...
  # SNR
  snr_index = np.argmax(snrs)
  item["num_series"] = 1
  item["bpm"] = bpms[snr_index]
  item["gap_seconds"] = items[snr_index]["gap_seconds"]
...
if __name__ == "__main__":
  ...
  sampling_rate = all_data["sampling_rate"].item()
  ...
  for method in rpeaks.keys():
    print("\n\nmethod: " + method)
    result[method] = []
    gaps = find_gaps(rpeaks[method], sampling_rate)
    total_gap = sum([gap.upper-gap.lower for gap in gaps]) / sampling_rate
    num_rpeaks = len(rpeaks[method]) - len(gaps)
    bpm = num_rpeaks / (total_sec - total_gap) * 60 if total_sec > total_gap else 0

    print(" total_gap in sec: " + str(total_gap) + " #rpeaks: " + str(num_rpeaks))

    for item in calculate_time_series_metrics(gaps, rpeaks[method], sampling_rate):
      result[method].append(item)

    pprint.pprint(result[method])

  if True:
    plot_bpm(result, all_data["inputfile"], sampling_rate)
